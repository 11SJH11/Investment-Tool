...
await evaluate(`document.querySelector('[aria-label="Saved runs"] tbody .row-actions button').click()`);await waitFor(`document.body.innerText.includes('Execution assumptions:')`);assert(await evaluate(`[...document.querySelectorAll('[aria-label="Backtest navigation"] button')].some(b=>b.textContent==='Run Viewer'&&b.getAttribute('aria-current')==='page')`),'Opening a saved run should enter Run Viewer');console.log('PASS two independent queued runs, raw NQ execution, saved result, comparison, Run Viewer');
const results=[];
for(const width of [1024,1440,1920]){
 await send('Emulation.setDeviceMetricsOverride',{width,height:1000,deviceScaleFactor:1,mobile:false});
 for(const [page,tabs,aria] of [['Journal',['Trades','Analysis','Calendar','Daily Review','Playbook'],'Journal navigation'],['Backtest',['Backtest','Runs','Strategies','Workspace'],'Backtest navigation']]){
