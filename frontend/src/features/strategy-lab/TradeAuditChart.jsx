...
export default function TradeAuditChart({ trade, timeframe: initialTimeframe, session: initialSession, onClose }) {
  ...
  useEffect(() => {
    ...
    if (showLevels) {
      const levels = [
        [trade.entry_price, "Entry", 2, 0], [trade.stop_loss, "Stop", 1, 2],
        [trade.take_profit, "Target", 1, 2], [trade.exit_price, "Exit", 1, 2],
      ];
    ...
    if (showMarkers) {
      ...
      if (entryTime != null && exitTime != null) {
        const markers = [
          { time: entryTime, position: trade.direction === "long" ? "belowBar" : "aboveBar", shape: trade.direction === "long" ? "arrowUp" : "arrowDown", color: "#60a5fa", text: `ENTRY ${money(trade.entry_price)}` },
          { time: exitTime, position: trade.direction === "long" ? "aboveBar" : "belowBar", shape: trade.direction === "long" ? "arrowDown" : "arrowUp", color: trade.result === "win" ? "#22c55e" : "#ef4444", text: `EXIT ${r(trade.r_multiple)}` },
        ];
        for (const partial of trade.metadata?.partial_exits || []) {
