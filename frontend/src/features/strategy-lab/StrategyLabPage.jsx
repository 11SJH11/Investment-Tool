...
export default function StrategyLabPage({ initialTab = "Backtest", standaloneTab = null, onWorkspaceDirty }) {
  ...
  return <div className={standaloneTab === "Replay" ? "w-full max-w-none" : "min-w-0 w-full max-w-[1600px]"}>
    ...
    {standaloneTab === "Backtest" && <div className="feature-nav" aria-label="Backtest navigation">{["Backtest","Runs","Strategies","Workspace"].map(item=><button key={item} aria-current={tab===item?"page":undefined} onClick={()=>{setTab(item);if(item==="Runs")refreshRuns()}} className={`mini-btn ${tab===item?"active-btn":""}`}>{item}</button>)}</div>}
      ...
      <section className="mt-5 rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
        <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
          <div>
            ...
            <Section id="batch-symbols" title="Add multiple symbols"><label className="text-xs">Paste symbols <textarea aria-label="Paste symbols" className="input" value={symbolInput} onChange={e=>setSymbolInput(e.target.value)} placeholder="AAPL, MSFT, NVDA"/></label><Button onClick={()=>{setSymbols(old=>[...new Set([...old,...parseSymbols(symbolInput)])]);setSymbolInput('');}}>Add pasted symbols</Button><p className="mt-2 text-xs text-stone-500">Independent symbol tests: one account and immutable run per symbol. This does not simulate a shared-capital portfolio.</p><Section id="batch-watchlist" title="Select watchlist symbols"><div className="ui-toolbar">{watchlist.items.map(symbol=><label key={symbol} className="text-xs"><input type="checkbox" checked={symbols.includes(symbol)} onChange={()=>setSymbols(old=>old.includes(symbol)?old.filter(s=>s!==symbol):[...old,symbol])}/> {symbol}</label>)}</div></Section></Section>
          </div>
          <div>
