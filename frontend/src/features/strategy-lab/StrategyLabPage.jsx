...
export default function StrategyLabPage({ initialTab = "Backtest", standaloneTab = null, onWorkspaceDirty }) {
  ...
  const autoRunName = useRef(true);
  const [runMeta, setRunMeta] = useState({ name: "Run 1", notes: "", tags: "", test_role: "development" });
  ...
  const refreshRuns = useCallback(() => api.strategyLabRuns(100).then((data) => {
    ...
    setRuns(items);
    if (autoRunName.current) setRunMeta((old) => ({ ...old, name: nextRunName(items) }));
    return items;
  }).catch(() => []), []);
  ...
  const addSymbol = (ticker) => {
    const values = parseSymbols(ticker || symbolInput);
    if (!values.length) return;
    setSymbols((old) => [...new Set([...old, ...values])]);
    setSymbolInput("");
  };
  ...
  const run = async () => {
    const issue = configurationError(buildPayload());
  ...
  const useSavedSettings = async (runId) => {
    ...
    try {
      ...
      autoRunName.current = false;
      setRunMeta({ name: saved.name ? `${saved.name} · copy` : nextRunName(runs), notes: saved.notes || "", tags: (saved.tags || []).join(", "), test_role: "development" });
    ...
    {tab === "Backtest" && <>
      ...
      <section className="mt-5 rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
        <div className="grid gap-4 border-b border-stone-100 pb-5 lg:grid-cols-2">
          <Field label="Run name · required"><input required className="input" value={runMeta.name} onChange={(e) => { autoRunName.current = false; setRunMeta({ ...runMeta, name: e.target.value }); }} placeholder="Run name" /></Field>
          <Field label="Tags · optional"><input className="input" value={runMeta.tags} onChange={(e) => setRunMeta({ ...runMeta, tags: e.target.value })} placeholder="baseline, AAPL, 5m" /></Field>
        </div>

        <div className="mt-5 grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
          <div>
            ...
            <div className="mt-4">
              <span className="mb-1 block text-xs font-medium text-stone-500">Symbols</span>
              <div className="flex flex-wrap gap-2">{symbols.map((symbol) => <button key={symbol} type="button" onClick={() => setSymbols((old) => old.filter((x) => x !== symbol))} className="rounded-full border border-stone-300 bg-stone-50 px-3 py-1 text-xs font-mono">{symbol} ×</button>)}</div>
              <div className="mt-2 max-w-md"><SymbolSearch value={symbolInput} onChange={setSymbolInput} onSelect={(item) => addSymbol(item.ticker)} placeholder="Add ticker(s): AAPL, MSFT, NVDA" /></div>
              <button type="button" onClick={() => addSymbol()} className="mt-2 text-xs font-medium text-stone-600 underline">Add typed symbol(s)</button>
              <p className="mt-2 text-[11px] text-stone-500">Comma, space or semicolon separated tickers create independent runs with independent capital.</p>
            </div>
          </div>
          <div>
            <h3 className="font-semibold">Account sizing</h3><p className="mt-1 text-xs text-stone-500">Controls position size. Strategy code continues to own stop, target and position-management logic.</p>
        ...
        <Section id="backtest-advanced" title="Advanced">
          <div className="grid gap-3 xl:grid-cols-2">
            <CompactAdvanced title="Execution">
              <FormGrid>
            ...
            <CompactAdvanced title="Costs & slippage">
              <div className="grid gap-3 sm:grid-cols-3"><Field label="Commission / order"><NumberInput value={form.commission_per_order} onChange={(v) => setForm({ ...form, commission_per_order: v })} /></Field><Field label="Slippage (bps)"><NumberInput value={form.slippage_bps} onChange={(v) => setForm({ ...form, slippage_bps: v })} /></Field><Field label="Full spread (bps)"><NumberInput value={form.spread_bps} onChange={(v) => setForm({ ...form, spread_bps: v })} /></Field></div>
            </CompactAdvanced>
            <CompactAdvanced title="Account limits">
              <div className="grid gap-3 sm:grid-cols-2"><Field label="Max leverage"><NumberInput value={form.max_leverage} onChange={(v) => setForm({ ...form, max_leverage: v })} /></Field><Field label="Max open positions"><NumberInput value={form.max_open_positions} onChange={(v) => setForm({ ...form, max_open_positions: v })} /></Field></div>
            </CompactAdvanced>
            <CompactAdvanced title="Guardrails">
              <div className="grid gap-3 sm:grid-cols-2"><Field label="Max trades / day"><OptionalNumberInput value={form.max_trades_per_day} onChange={(v) => setForm({ ...form, max_trades_per_day: v })} placeholder="No limit" /></Field><Field label="Daily loss stop (R)"><OptionalNumberInput value={form.max_daily_loss_r} onChange={(v) => setForm({ ...form, max_daily_loss_r: v })} placeholder="No limit" /></Field><Field label="Consecutive loss stop"><OptionalNumberInput value={form.max_consecutive_losses} onChange={(v) => setForm({ ...form, max_consecutive_losses: v })} placeholder="No limit" /></Field><Field label="Cooldown after exit (min)"><NumberInput value={form.cooldown_minutes} onChange={(v) => setForm({ ...form, cooldown_minutes: v })} /></Field></div>
            </CompactAdvanced>
          </div>
          <CompactAdvanced title="Trading schedule" className="mt-3">
            <p className="text-xs text-stone-500">No entry window means all loaded market hours are eligible. Optional windows use New York / ET, start-inclusive and end-exclusive.</p>
            <div className="mt-3 grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
              <div><p className="text-xs font-medium text-stone-500">Allowed entry windows <span className="font-normal">(optional)</span></p>{!entryWindows.length && <p className="mt-2 text-xs text-stone-400">None · entries may occur at any loaded market time.</p>}<div className="mt-2 space-y-2">{entryWindows.map((window, index) => <div key={index} className="flex max-w-xl items-center gap-2"><input type="time" className="input" value={window.start} onChange={(e) => setEntryWindows((old) => old.map((w,i) => i === index ? { ...w, start: e.target.value } : w))} /><span className="text-xs text-stone-500">to</span><input type="time" className="input" value={window.end} onChange={(e) => setEntryWindows((old) => old.map((w,i) => i === index ? { ...w, end: e.target.value } : w))} /><button type="button" onClick={() => setEntryWindows((old) => old.filter((_,i) => i !== index))} className="px-2 text-xs text-red-700">Remove</button></div>)}</div><button type="button" onClick={() => setEntryWindows((old) => [...old, { start: "09:30", end: "16:00" }])} className="mt-2 text-xs font-medium text-stone-600 underline">+ Add window</button></div>
              <div><p className="text-xs font-medium text-stone-500">Trading days</p><div className="mt-2 flex flex-wrap gap-2">{weekdays.map(([value,label]) => <button type="button" key={value} onClick={() => setSelectedWeekdays((old) => old.includes(value) ? old.filter((x) => x !== value) : [...old, value].sort())} className={`rounded-md border px-3 py-2 text-xs ${selectedWeekdays.includes(value) ? "border-stone-900 bg-stone-900 text-white" : "border-stone-300 bg-white text-stone-600"}`}>{label}</button>)}</div><div className="mt-3 grid gap-3 sm:grid-cols-2"><Field label="Hold overnight?"><select className="input" value={form.allow_overnight} onChange={(e) => setForm({ ...form, allow_overnight: e.target.value })}><option value="false">No · flatten same day</option><option value="true">Yes · allow overnight</option></select></Field>{form.allow_overnight === "false" && <Field label="Force close time (ET)"><input type="time" className="input" value={form.force_close_time} onChange={(e) => setForm({ ...form, force_close_time: e.target.value })} /></Field>}</div></div>
            </div>
          </CompactAdvanced>
        </Section>
        ...
        <div className="ready-summary"><h3 className="font-semibold">{configurationError(buildPayload())?"Check configuration":"Ready to run"}</h3>{configurationError(buildPayload())&&<p role="alert" className="mt-2 text-sm text-red-700">{configurationError(buildPayload())}</p>}<p className="mt-1 text-sm">{strategy?.name}</p><MetricGrid>{Object.entries(runSummary(buildPayload())).map(([label,value])=><MetricCard key={label} label={label}>{value}</MetricCard>)}</MetricGrid><p className="mt-3 text-xs text-stone-500">Requested date range; actual end is capped at the provider's latest available data and recorded in the saved result.</p></div>
...
function BacktestResults({ result, onUseSettings, onBackToRuns }) {
  ...
  const visibleTrades = showAllTrades ? filtered : filtered.slice(0, 20);
  ...
  return <section className="mt-5">
    ...
    {viewerTab === "Summary" && <>
      ...
      <section className="mt-5 rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">Performance</h3><p className="mt-1 text-xs text-stone-500">Hover for exact values. Click a trade marker to audit that trade.</p></div><div className="flex flex-wrap items-end gap-2"><Field label="Timezone"><select className="input min-w-[190px]" value={chartZone} onChange={(e) => setChartZone(e.target.value)}>{TIMEZONE_OPTIONS.map((zone) => <option key={zone.value} value={zone.value}>{zone.label}</option>)}</select></Field><span className="pb-2 text-xs text-stone-500">Data: {String(result.data?.feed || "—").toUpperCase()} · adjustment {result.data?.adjustment || "—"}</span></div></div>
...
function CompactAdvanced({ title, children, className = "" }) {
  return <div className={`rounded-lg border border-stone-200 bg-stone-50 p-3 ${className}`}><h4 className="mb-3 text-xs font-semibold uppercase tracking-wide text-stone-500">{title}</h4>{children}</div>;
}
