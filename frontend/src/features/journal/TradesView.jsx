...
export default function TradesView({timeZone,playbooks,options,revision,onChanged,onDirtyChange,report,loading,error:loadError,columns,table}) {
  ...
  return <div className="space-y-5"><Summary data={report?.summary}/><div className="ui-toolbar"><span className="muted text-sm">{rows.length} matching trades</span>{['table','cards'].map(v=><button key={v} className={`mini-btn ${view===v?'active-btn':''}`} onClick={()=>setView(v)}>{v==='cards'?'Cards':'Table'}</button>)}<button className="mini-btn ledger-primary text-white" onClick={()=>navigate(()=>{setCreating(true);setSelected(null);})}>+ Log trade</button></div>
    ...
    {selected&&<TradeDetail key={selected.id} trade={selected} timeZone={timeZone} playbooks={playbooks} options={options} revision={revision} onDirtyChange={setDirty} onClose={()=>navigate(()=>setSelected(null))} onChanged={onChanged}/>}
    {loading&&<p role="status" className="muted text-sm">Updating filtered trades…</p>}
    {view==='table'?<DataTable id="journal" label="Journal trades" rows={rows} columns={columns} controller={table} manual primaryAction={{label:'Open',onClick:t=>navigate(()=>void open(t.id))}} actions={t=>[{label:'Open chart at trade',onClick:()=>openWorkflow('Charts',{...t,timeframe:'5m'})},{label:'Replay this period',onClick:()=>openWorkflow('Replay',{...t,timeframe:'5m'})},...(t.source_metadata?.strategy_key?[{label:'Open strategy',onClick:()=>openWorkflow('Backtest',{...t,strategy_key:t.source_metadata.strategy_key})}]:[]),{label:'Delete',onClick:()=>navigate(()=>void remove(t))}]}/>:<div className="grid gap-4 lg:grid-cols-2">{rows.map(t=><article key={t.id} className="insight-panel"><div className="flex justify-between gap-3"><button className="text-lg font-semibold text-blue-700" onClick={()=>navigate(()=>void open(t.id))}>{t.ticker} · {t.direction}</button><span>{t.setup_grade&&`Grade ${t.setup_grade}`}</span></div><p className="mt-2 text-xs muted">{sourceLabel(t.source)} · {t.account} · {formatInZone(t.opened_at,timeZone)}</p><div className="mt-3 ui-toolbar"><span className={resultTone(t.result)}>{t.result||t.status}</span><span>{moneyCurrency(t.pnl_amount,t.account_currency||t.position_currency||'USD')}</span><span>{rValue(t.r_multiple)}</span></div><div className="mt-3 ui-toolbar"><button className="mini-btn" onClick={()=>navigate(()=>void open(t.id))}>Open review</button><button className="mini-btn" onClick={()=>openWorkflow("Charts",{...t,timeframe:"5m"})}>Open chart at trade</button><button className="mini-btn" onClick={()=>openWorkflow("Replay",{...t,timeframe:"5m"})}>Replay this period</button><button className="mini-btn" onClick={()=>navigate(()=>void remove(t))}>Delete</button></div></article>)}</div>}
  </div>;
}
