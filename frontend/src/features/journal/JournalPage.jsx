...
import {useJournalData} from './useJournalData.js';
...
export default function JournalPage({onDirtyChange}) {
  ...
  const openDay=(date,target)=>navigate(()=>{const accounts=[JSON.parse(filters.dimensions_json||'{}').account,table.view.filters.account,[...new Set((data.report?.trades||[]).filter(t=>t.journal_date===date).map(t=>t.account))]].find(values=>Array.isArray(values)&&values.length)||[];setDay({date,account:accounts.length===1?accounts[0]:'Main'});if(target==='Trades')setFilters({...filters,date_from:date,date_to:date});setTab(target);});
  const columns=useMemo(()=>journalColumns(zone||'UTC',options),[zone,options]);
  const table=useTableView('journal',columns,[],{sort:{key:'opened_at',direction:'desc'}});
  const query={...Object.fromEntries(Object.entries(filters).filter(([,v])=>v!=='')),timezone:zone,table_filters_json:JSON.stringify(table.view.filters),table_sort_json:JSON.stringify(table.view.sort),search:table.view.search};
  const data=useJournalData(query,revision,Boolean(zone));
  const resetView=()=>navigate(()=>{table.reset();setFilters({...emptyFilters});resetUI(['journal.view','section.journal-statistics']);});
