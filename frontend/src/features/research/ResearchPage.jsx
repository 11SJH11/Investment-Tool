...
export default function ResearchPage({ selectedTicker = "AAPL", onTickerChange }) {
  ...
  const [researchDraft, setResearchDraft] = useState({ source: "financial_times", title: "", url: "", summary: "", direction: "neutral" });
  ...
  const loadResearchItems = async () => {
    ...
    try {
      const result = await api.researchItems({ instrument: ticker, limit: 50 });
  ...
  const saveResearchNote = async (event) => {
    event.preventDefault();
    if (!researchDraft.title.trim()) return;
    setResearchSaving(true); setResearchMessage("");
    try {
      await api.saveResearchItem({
        source: researchDraft.source, item_type: "note", instrument: ticker,
        title: researchDraft.title.trim(), url: researchDraft.url.trim(),
        summary: researchDraft.summary.trim(), direction: researchDraft.direction,
        metadata: { capture_method: "manual", note: "No article content was scraped or fetched by Ledger." },
      });
      setResearchDraft((old) => ({ ...old, title: "", url: "", summary: "", direction: "neutral" }));
      setResearchMessage("Research note saved.");
      await loadResearchItems();
    } catch (e) { setError(e.message); }
    finally { setResearchSaving(false); }
  };

  const deleteResearchNote = async (id) => {
    try { await api.deleteResearchItem(id); await loadResearchItems(); }
    catch (e) { setError(e.message); }
  };
  ...
  useEffect(() => { if (ticker) { loadProfile(); loadBars(); loadResearchItems(); } }, [ticker]);
    ...
    <div className="max-w-[1450px]">
      ...
      <section className={`${expanded ? "fixed inset-3 z-50 mt-0" : "mt-6"} overflow-hidden rounded-xl border border-stone-200 bg-white shadow-xl`}>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-stone-200 px-4 py-2 text-[11px] text-stone-500">
          <span>Data: {String(barMeta.provider || "provider").toUpperCase()} {barMeta.feed ? String(barMeta.feed).toUpperCase() : ""} · {barMeta.historical_delay_minutes ? `~${barMeta.historical_delay_minutes} min delayed` : "provider timing"} · {barMeta.session === "24h" ? "24h / full market" : barMeta.session === "regular" ? "regular session" : "extended session"}{barMeta.aggregation === "session_aligned_from_30m" ? " · session-aligned from 30m bars" : ""}{barMeta.adjustment ? ` · adjustment: ${barMeta.adjustment}` : ""}</span>
      ...
      {symbol && (
        <section className="mt-5 rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-baseline justify-between gap-3">
            <div><h3 className="font-semibold">Research workspace</h3><p className="mt-1 text-xs text-stone-500">Save the source, link and your interpretation. Ledger does not scrape Financial Times, OANDA or Autochartist content.</p></div>
            <span className="text-xs text-stone-500">{researchLoading ? "Loading…" : `${researchItems.length} saved for ${ticker}`}</span>
          </div>
          <form onSubmit={saveResearchNote} className="mt-4 grid gap-3 lg:grid-cols-6">
            <label className="block"><span className="mb-1 block text-xs font-medium text-stone-500">Source</span><select className="input" value={researchDraft.source} onChange={(e) => setResearchDraft({ ...researchDraft, source: e.target.value })}><option value="financial_times">Financial Times</option><option value="autochartist_portal">Autochartist portal</option><option value="oanda">OANDA</option><option value="manual">Manual / other</option></select></label>
            <label className="block lg:col-span-2"><span className="mb-1 block text-xs font-medium text-stone-500">Title</span><input className="input" required value={researchDraft.title} onChange={(e) => setResearchDraft({ ...researchDraft, title: e.target.value })} placeholder="Article, thesis or catalyst" /></label>
            <label className="block lg:col-span-2"><span className="mb-1 block text-xs font-medium text-stone-500">Link</span><input className="input" type="url" value={researchDraft.url} onChange={(e) => setResearchDraft({ ...researchDraft, url: e.target.value })} placeholder="https://…" /></label>
            <label className="block"><span className="mb-1 block text-xs font-medium text-stone-500">Bias</span><select className="input" value={researchDraft.direction} onChange={(e) => setResearchDraft({ ...researchDraft, direction: e.target.value })}><option value="neutral">Neutral</option><option value="bullish">Bullish</option><option value="bearish">Bearish</option><option value="mixed">Mixed</option><option value="unknown">Unknown</option></select></label>
            <label className="block lg:col-span-5"><span className="mb-1 block text-xs font-medium text-stone-500">Your summary / thesis</span><textarea className="input min-h-20" value={researchDraft.summary} onChange={(e) => setResearchDraft({ ...researchDraft, summary: e.target.value })} placeholder="What matters, catalyst, risks, what would invalidate the idea…" /></label>
            <div className="flex items-end"><button disabled={researchSaving} className="ledger-primary w-full rounded-md px-4 py-2 text-sm text-white disabled:opacity-50">{researchSaving ? "Saving…" : "Save note"}</button></div>
          </form>
          {researchMessage && <p className="mt-2 text-xs text-emerald-700">{researchMessage}</p>}
          <div className="mt-4 space-y-2">
            {researchItems.length === 0 && !researchLoading ? <p className="text-xs text-stone-500">No saved research for this instrument yet.</p> : researchItems.map((item) => <div key={item.id} className="rounded-lg border border-stone-200 bg-stone-50 p-3"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] uppercase tracking-wide text-stone-500">{String(item.source || "manual").replaceAll("_", " ")} · {String(item.direction || "neutral")}</p><p className="mt-1 font-medium">{item.url ? <a className="underline decoration-stone-300 underline-offset-2" href={item.url} target="_blank" rel="noreferrer">{item.title}</a> : item.title}</p>{item.summary && <p className="mt-1 whitespace-pre-wrap text-sm text-stone-600">{item.summary}</p>}<p className="mt-1 text-[11px] text-stone-400">{formatStamp(item.published_at || item.created_at)}</p></div><button type="button" onClick={() => deleteResearchNote(item.id)} className="text-xs text-stone-500 underline">Delete</button></div></div>)}
          </div>
        </section>
      )}
